...
import { Search, Calendar, Mic, User, Pin, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 py-8">
      <div className="text-center space-y-3">
        ...
        <h1 className="text-3xl font-extrabold text-white">Global Instant Search</h1>
        <p className="text-sm text-slate-400">Search across events, guest speakers, core team members, and community notices.</p>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-4 text-sm bg-slate-900 border border-slate-800 rounded-2xl text-white placeholder-slate-500 focus:ring-2 focus:ring-sky-500 focus:outline-none shadow-2xl"
        />
      </div>

      {query && (
        <div className="space-y-6">
          <p className="text-xs font-semibold text-slate-400">Found {totalResults} matching results for "{query}"</p>

          {/* Events Results */}
          {matchingEvents.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-sky-400">Events ({matchingEvents.length})</h3>
              {matchingEvents.map((evt) => (
                <Card key={evt.id} className="p-4 flex items-center justify-between gap-4 border-slate-800">
                  <div>
                    <h4 className="text-sm font-bold text-white">{evt.title}</h4>
                    <p className="text-xs text-slate-400">{evt.category} • {evt.venue}</p>
                  </div>
          ...
          {matchingSpeakers.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-xs font-bold uppercase tracking-wider text-purple-400">Speakers ({matchingSpeakers.length})</h3>
              {matchingSpeakers.map((spk) => (
                <Card key={spk.id} className="p-4 flex items-center justify-between gap-4 border-slate-800">
                  <div>
                    <h4 className="text-sm font-bold text-white">{spk.name}</h4>
                    <p className="text-xs text-slate-400">{spk.designation} - {spk.organization}</p>
                  </div>
