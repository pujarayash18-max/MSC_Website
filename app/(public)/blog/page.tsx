  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10 py-8">
      <div className="text-center max-w-3xl mx-auto space-y-3">
        ...
        <h1 className="text-4xl font-extrabold text-white">Engineering & Cloud Insights</h1>
        <p className="text-sm text-slate-400">Articles, tutorials, and cloud architecture deep dives written by MCC leads and guest speakers.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {MOCK_BLOGS.map((blog) => (
          <Card key={blog.blogId} className="overflow-hidden border-slate-800 flex flex-col group hover:border-sky-500/50 transition-all">
            ...
            <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-white group-hover:text-sky-400 transition-colors">{blog.title}</h3>
                <p className="text-xs text-slate-400 leading-relaxed">{blog.excerpt}</p>
              </div>

              <div className="pt-4 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
                <div className="flex items-center gap-3">
                  <span className="flex items-center gap-1 text-white font-medium"><User className="w-3.5 h-3.5 text-sky-400" /> {blog.author}</span>
                  <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-slate-500" /> {blog.readingTime}</span>
                </div>

                <Link href={`/blog/${blog.slug}`}>
                  <Button variant="ghost" size="sm" className="text-sky-400 hover:text-white">
