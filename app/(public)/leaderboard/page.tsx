  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 py-8">
      <div className="text-center max-w-3xl mx-auto space-y-3">
        ...
        <p className="text-sm text-slate-400">Recognizing student excellence, workshop participation, and hackathon achievements.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto items-end">
        {/* 2nd Place */}
        <Card className="p-6 text-center space-y-3 border-slate-700 bg-gradient-to-b from-slate-900 to-slate-950 shadow-xl order-2 md:order-1">
          <div className="w-12 h-12 rounded-full bg-slate-400/20 border border-slate-400 text-slate-200 font-bold flex items-center justify-center mx-auto">
            #2
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{top3[1].dept}</p>
          </div>
          <Badge variant="primary" className="text-sm font-extrabold">{top3[1].points} pts</Badge>
        </Card>

        {/* 1st Place Champion */}
        <Card className="p-8 text-center space-y-4 border-amber-500/50 bg-gradient-to-b from-slate-900 via-amber-950/20 to-slate-950 shadow-2xl scale-105 order-1 md:order-2">
          ...
          <div>
            ...
            <h3 className="text-xl font-extrabold text-white">{top3[0].name}</h3>
            <p className="text-xs text-amber-300 font-semibold">{top3[0].dept}</p>
          </div>
          <div className="text-2xl font-black text-amber-400">{top3[0].points} pts</div>
        </Card>

        {/* 3rd Place */}
        <Card className="p-6 text-center space-y-3 border-orange-500/30 bg-gradient-to-b from-slate-900 to-slate-950 shadow-xl order-3">
          <div className="w-12 h-12 rounded-full bg-orange-500/20 border border-orange-400 text-orange-300 font-bold flex items-center justify-center mx-auto">
            #3
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{top3[2].dept}</p>
          </div>
      ...
      <Card className="max-w-5xl mx-auto p-6 space-y-4 border-slate-800">
        <h3 className="text-base font-bold text-white border-b border-slate-800 pb-3">Ranked Student Members</h3>
        <div className="space-y-3">
          {MOCK_LEADERBOARD.map((item) => (
            <div key={item.rank} className="p-4 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between gap-4 hover:border-slate-700 transition-colors">
              <div className="flex items-center gap-4">
                <span className="w-8 text-center font-extrabold text-slate-400 text-sm">#{item.rank}</span>
                <img src={item.photo} alt={item.name} className="w-10 h-10 rounded-xl object-cover border border-slate-700" />
                <div>
                  <h4 className="text-sm font-bold text-white">{item.name}</h4>
                  <p className="text-xs text-slate-400">{item.dept} • {item.college}</p>
                </div>
              </div>

              <div className="flex items-center gap-6">
                <div className="text-right text-xs">
                  <span className="text-slate-400">Badges: <strong className="text-purple-400">{item.badges}</strong></span>
                  <p className="text-slate-400">Events Won: <strong className="text-emerald-400">{item.won}</strong></p>
                </div>
