      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-xs bg-slate-900 border border-slate-800 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500"
          />
        </div>
      </div>
        ...
        {filteredEvents.map((evt) => (
          <Card key={evt.id} className="flex flex-col h-full overflow-hidden">
            ...
            <div className="p-6 flex flex-col flex-1 space-y-4">
              ...
              <h2 className="text-lg font-bold text-white line-clamp-2">{evt.title}</h2>
              <p className="text-xs text-slate-400 line-clamp-2">{evt.shortDescription}</p>

              <div className="pt-2 border-t border-slate-800 space-y-2 text-xs text-slate-300 mt-auto">
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-sky-400" /> Aug 25, 2026</span>
                  <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-sky-400" /> Full Day</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-sky-400" /> {evt.venue}</span>
                  <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-emerald-400" /> {evt.remainingSeats} Seats Left</span>
                </div>
              </div>
